...
function App() {
  ...
  const handlePointerMove = (x: number, y: number, e: React.PointerEvent) => {
    e.preventDefault();
    if (dragState.currentCoord?.x !== x || dragState.currentCoord?.y !== y) {
      setDragState(prev => ({ ...prev, currentCoord: { x, y } }));
    }
  };
  ...
  const handlePointerUp = (x: number, y: number, e: React.PointerEvent) => {
    ...
    if (dragState.isDragging && dragState.planeId) {
      const isClick = dragState.startCoord?.x === x && dragState.startCoord?.y === y;
      ...
      if (isClick) {
        ...
        updatePlane(movingPlaneId, { x, y }, nextDir, dragState.isGhost);
      } else {
        updatePlane(movingPlaneId, { x, y }, currentPlane.direction, dragState.isGhost);
      }
    } 
    else if (!dragState.isDragging && dragState.startCoord?.x === x && dragState.startCoord?.y === y) {
      if (isSetup) {
        placeNewPlane(x, y);
      } else if (isDeduce) {
        const clickedPlane = activePlayer.ghostPlanes.find(p => p.cells.some(c => c.x === x && c.y === y));
        if (!clickedPlane) placeNewGhost(x, y);
                        ...
                        <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/20 backdrop-blur-[2px] animate-in fade-in duration-300">
                             <div className="bg-black/80 p-6 rounded-2xl border border-white/20 text-center transform scale-110 shadow-2xl">
                                <div className={`text-4xl font-black mb-2 ${
                                    logs[0]?.result === 'KILL' ? 'text-red-500 animate-bounce' : 
